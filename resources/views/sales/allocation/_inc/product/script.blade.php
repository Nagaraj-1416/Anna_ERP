...
<script>
    app.controller('AddProductController', function ($scope, $http) {
        ...
        $scope.el.productDD.dropdown('setting', {
            ...
            apiSettings: {
                url: '{{ route('sales.allocation.get.products', [$allocation]) }}' + '/{query}',
                cache: false
            },
        ...
        $scope.addProductRoute = '{{ route('sales.allocation.add.product', [$allocation])}}';
        ...
        $scope.initStoreDropDown = function (el, productId) {
            ...
            //el.dropdown('set text', '{{ $model->company->name }}').dropdown('set value', '{{ $model->company->id }}')
        };
