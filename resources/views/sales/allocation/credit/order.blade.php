...
@section('breadcrumbs')
    {!! breadcrumbRender($breadcrumb, 'Attach Credit Orders') !!}
@endsection
@section('content')
        ...
        <div class="col-12">
            <div class="card card-outline-info">
                <div class="card-header">
                    <h3 class="text-white">Attach orders</h3>
                </div>
                <div class="card-body">
                    {!! form()->model($allocation, ['url' => route('sales.allocation.attach.credit.order', $allocation), 'method' => 'POST']) !!}
                    <div class="form-body">
                        <div>
                            <hr>
                            <h4 class="box-title">Credit Orders</h4>
                            ...
                            <div class="m-b-10">
                                <input type="text" style="margin-left: 0 !important;" id="demo-input-search2"
                                       ...
                                       placeholder="search for order here" class="form-control" autocomplete="off">
                            </div>
                                        ...
                                        <td style="width: 2%;">
                                            <div class="demo-checkbox">
                                                <input type="checkbox" id="@{{ 'md_checkbox_28_' + order.id }}"
                                                       ...
                                                       class="chk-col-cyan order-check"
                                                       data-order="@{{ order.id }}"
                                                       ng-click="updateCheckBox(order.id)">
...
@section('script')
    ...
    <script>
        app.controller('AllocationController', ['$scope', '$http', function ($scope, $http) {
            $scope.orders = _.toArray(@json($orders));
            $scope.checkedOrders = {};
            $scope.handleCustomerCheckAll = function ($event) {
                if ($($event.target).is(':checked')) {
                    $('.order-check').prop('checked', true);
                    $.each($scope.orders, function (index, value) {
                        $scope.updateCheckBox(value.id, true);
                    });
                } else {
                    $('.order-check').prop('checked', false);
                    $.each($scope.orders, function (index, value) {
                        $scope.updateCheckBox(value.id, false);
                    });
                }
            };

            $scope.updateCheckBox = function (id, update) {
                if ($scope.checkedOrders.hasOwnProperty(id)) {
                    delete $scope.checkedOrders[id];
                } else {
                    $scope.checkedOrders[id] = true;
                }
            ...
            return function (scope, element, attr) {
                if (scope.checkedOrders.hasOwnProperty(scope.order.id)) {
                    element.find('.order-check').prop('checked', true);
                }
            };
        });
