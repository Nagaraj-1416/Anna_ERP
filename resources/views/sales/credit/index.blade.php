            ...
            <div class="card">
                ...
                <div class="contact-page-aside">
                    ...
                    <div class="left-aside">
                        @can('create', new \App\CustomerCredit())
                            <a target="_blank" href="{{ route('sales.credit.create') }}" class="btn btn-info btn-block">
                                ...
                                <div class="card card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-9">
                                            ...
                                            <small>@{{ credit.customer.display_name }} | @{{credit.customer.mobile}}
                                            </small>
                                            <p class="text-muted">
                                ...
                                <div class="card text-center">
                                    <div class="card-body">
                                        <p class="card-text">You haven't added any credits yet, click below button to
                                            add.</p>
                                        <a target="_blank" href="{{ route('sales.credit.create') }}"
                                           class="btn btn-info">
                                            <i class="fa fa-plus"></i> Add New Credit
                                        </a>
                                    </div>
...
@section('script')
    <script>
        app.controller('CreditController', ['$scope', '$http', function ($scope, $http) {
            var moduleRoute = '{{ route('sales.credit.index') }}';
            $scope.credits = [];
            $scope.filterd = false;
            $scope.loading = true;
            $scope.pagination = {};
            ...
            var openCreditRoute = '{{ route('sales.summary.index', [
                'model' => 'CustomerCredit', 'take' => 'null', 'with' => 'null', 'where' => 'Open', 'field' => 'status']) }}';
            ...
            var closedCreditRoute = '{{ route('sales.summary.index', [
                'model' => 'CustomerCredit', 'take' => 'null', 'with' => 'null', 'where' => 'Closed', 'field' => 'status']) }}';
            $http.get(closedCreditRoute + '?ajax=true').then(function (response) {
                $scope.closedCreditCount = response.data ? response.data.count : 0;
            });

            /** get canceled credit count */
            var canceledCreditRoute = '{{ route('sales.summary.index', [
                'model' => 'CustomerCredit', 'take' => 'null', 'with' => 'null', 'where' => 'Canceled', 'field' => 'status']) }}';
