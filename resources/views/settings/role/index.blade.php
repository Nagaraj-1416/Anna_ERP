...
@endsection
@section('content')
    <div class="row" ng-controller="RoleController">
        <div class="col-12">
            <div class="card">
                ...
                <div class="contact-page-aside">
                    ...
                    <div class="left-aside">
                        <a target="_blank" href="{{ route('setting.role.create') }}" class="btn btn-info btn-block">
                            <i class="fa fa-plus"></i> Add New Role
                        </a>
                        <ul class="list-style-none">
                            <li class="text-muted m-t-20">FILTER ROLE BY</li>
                            <li class="divider"></li>
                    ...
                    <div class="right-aside custom-right-aside">
                        <div class="right-page-header">
                            <div class="d-flex m-b-10">
                                <div class="align-self-center">
                                    <h2 class="card-title m-t-10">Roles @{{ total ? ("(" + total +")") :
                                        '' }}</h2>
                                </div>
                                <div class="ml-auto">
                                    <input type="text" id="demo-input-search2" ng-model="searchRoles"
                                           placeholder="search for role here" class="form-control"
                                ...
                                <table id="demo-foo-addrow" class="table m-t-10 table-hover no-wrap contact-list" data-page-size="10">
                                    ...
                                    <tr ng-repeat="role in roles">
                                        <td style="vertical-align: middle;">
                                            <a target="_blank" href="/setting/role/@{{ role.id }}">
                                                @{{ role.name }}
                                            </a><br />
                                            <small>@{{ role.description }}</small>
                                        </td>
                                        <td class="text-center">@{{ role.access_level }}</td>
                                        <td class="text-center">@{{ role.users.length }}</td>
                                        <td class="text-center">
                                            <a href="/setting/role/@{{ role.id }}/edit">
                        ...
                        <div class="row" ng-hide="loading" ng-if="roles.length === 0 && !filterd">
                            <div class="col-md-12">
                                <div class="card text-center">
                                    <div class="card-body">
                                        <p class="card-text">You haven't added any role yet, click on "Add New Role" button to add role.</p>
                                        <a target="_blank" href="{{ route('setting.role.create') }}"
                                           class="btn btn-info">
                                            <i class="fa fa-plus"></i> Add New Role
                                        </a>
                                    </div>
                        ...
                        <div class="row" ng-hide="loading" ng-if="roles.length === 0 && filterd">
                            <div class="col-md-12">
                                <div class="card text-center">
                                    <div class="card-body">
                                        <p class="card-text">There are <code>no</code> roles found.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
...
@section('script')
    <script>
        app.controller('RoleController', ['$scope', '$http', function ($scope, $http) {
            var moduleRoute = '{{ route('setting.role.index') }}';
            $scope.roles = [];
            $scope.filterd = false;
            $scope.loading = true;
            $scope.pagination = {};
            ...
            $scope.filterUpdated = function () {
                $scope.filterd = true;
                $scope.query.search = $scope.searchRoles;
                $scope.fetchRoles();
            };
