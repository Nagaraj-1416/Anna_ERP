            ...
            <div class="card">
                ...
                <div class="contact-page-aside">
                    ...
                    <div class="left-aside">
                        <a target="_blank" href="{{ route('setting.company.create') }}" class="btn btn-info btn-block">
                            <i class="fa fa-plus"></i> Add New Company
                        </a>
                        <ul class="list-style-none">
                            <li class="text-muted m-t-20">FILTER COMPANY BY</li>
                            <li class="divider"></li>
                            <li ng-class="{'active': !query.filter}"
                                ng-click="filterUpdate()"><a
                                        href="">All Companies</a></li>
                            <li ng-class="{'active': query.filter === 'Active'}"
                                ng-click="filterUpdate('Active')"><a href="">Active Companies</a></li>
                            <li ng-class="{'active': query.filter === 'Inactive'}"
                                ng-click="filterUpdate('Inactive')"><a href="">Inactive Companies</a></li>
                    ...
                    <div class="right-aside custom-right-aside">
                        <div class="right-page-header">
                            <div class="d-flex m-b-10">
                                <div class="align-self-center">
                                    <h2 class="card-title m-t-10">Companies @{{ total ? ("(" + total +")") :
                                        '' }}</h2>
                                </div>
                                <div class="ml-auto">
                                    <input type="text" id="demo-input-search2" ng-model="searchCompanies"
                                           placeholder="search for company here" class="form-control"
                                ...
                                <table id="demo-foo-addrow" class="table m-t-10 table-hover no-wrap contact-list" data-page-size="10">
                                    ...
                                    <tr ng-repeat="company in companies">
                                        <td style="vertical-align: middle;">
                                            <a target="_blank" href="company/@{{ company.id }}">
                                                @{{ company.name }}
                                            </a><br />
                                            <small>
                                                @{{  company.code }} |
                                                <i class="mdi mdi-email"></i> @{{ company.email }}
                                                <i class="mdi mdi-phone-classic"></i> @{{ company.phone }}
                                                <i class="mdi mdi-cellphone"></i> @{{ company.mobile }}
                                                <i class="mdi mdi-fax"></i> @{{ company.fax }}
                                            </small>
                                        </td>
                                        <td>@{{ company.business_location }}</td>
                                        <td>@{{ company.staff.length }}</td>
                                        <td>
                                            <span class="label label-success" ng-if="company.is_active == 'Yes'">Active</span>
                                            <span class="label label-danger" ng-if="company.is_active == 'No'">Inactive</span>
                                        </td>
                        ...
                        <div class="row" ng-hide="loading" ng-if="companies.length === 0 && !filterd">
                            <div class="col-md-12">
                                <div class="card text-center">
                                    <div class="card-body">
                                        <p class="card-text">You haven't added any company yet, click on "Add New
                                            Company" button to add company.</p>
                                        <a target="_blank" href="{{ route('setting.company.create') }}"
                                           class="btn btn-info">
                                            <i class="fa fa-plus"></i> Add New Company
                                        </a>
                                    </div>
                        ...
                        <div class="row" ng-hide="loading" ng-if="companies.length === 0 && filterd">
                            <div class="col-md-12">
                                <div class="card text-center">
                                    <div class="card-body">
                                        <p class="card-text">There are <code>no</code> companies found.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
...
@section('script')
    <script>
        app.controller('CompanyController', ['$scope', '$http', function ($scope, $http) {
            var moduleRoute = '{{ route('setting.company.index') }}';
            $scope.companies = [];
            $scope.filterd = false;
            $scope.loading = true;
            $scope.pagination = {};
