<div class="form-body" ng-controller="ExpenseFormController">
    <div class="row">
        <div class="col-md-3">
            {!! form()->bsText('expense_date', 'Payment date', null, ['placeholder' => 'pick payment date', 'class' => 'form-control datepicker', 'ng-model' => 'todayDate']) !!}
        </div>
        <div class="col-md-3">
            {!! form()->bsText('expense_time', 'Payment time', null, ['placeholder' => 'pick payment time', 'class' => 'form-control clockpicker', 'ng-model' => 'currentTime']) !!}
        </div>
        <div class="col-md-3">
            {!! form()->bsText('amount', 'Amount', null, ['placeholder' => 'enter payment amount']) !!}
        </div>
    </div>
    <hr>
        ...
        <div class="col-md-3">
            <div class="form-group required">
                ...
                <div class="demo-radio-button">
                    <input name="expense_mode" value="Own" type="radio" class="with-gap expense-mode"
                           id="Own" checked {{ (old('expense_mode') == 'Own') || (isset($expense) && $expense->expense_mode == 'Own' ) ? 'checked' : ''}}>
                    <label for="Own">Own Payment</label>
                    <input name="expense_mode" value="ForOthers" type="radio" class="with-gap expense-mode"
                           id="ForOthers" {{ (old('expense_mode') == 'ForOthers') || (isset($expense) && $expense->expense_mode == 'ForOthers' )  ? 'checked' : ''}}>
                    <label for="ForOthers">For Others</label>
                </div>
            </div>
            <input type="hidden" value="Own" class="expense-mode-hidden">
        </div>
        <div class="col-md-3">
            <div class="form-group required">
                <label class="control-label">Payment Category</label>
                <div class="demo-radio-button">
                    <input name="expense_category" value="Office" type="radio" class="with-gap expense-category disabled"
                           id="Office" {{ (old('expense_category') == 'Office') || (isset($expense) && $expense->expense_category == 'Office' ) ? 'checked' : ''}}>
                    <label for="Office">Office</label>
                    <input name="expense_category" value="Shop" type="radio" class="with-gap expense-category disabled"
                           id="Shop" {{ (old('expense_category') == 'Shop') || (isset($expense) && $expense->expense_category == 'Shop' )  ? 'checked' : ''}}>
                    <label for="Shop">Shop</label>
                </div>
            </div>
            <input type="hidden" class="expense-category-hidden">
        </div>
    </div>
    <hr />
    <div class="row">
        <div class="col-md-3">
            <div class="form-group required {{ $errors->has('company_id') ? 'has-danger' : '' }}">
                ...
                <div class="ui fluid search normal selection dropdown company-drop-down {{ $errors->has('company_id') ? 'error' : '' }}">
                    @if(isset($expense))
                        <input name="company_id" class="company-value" type="hidden" value="{{ old('_token') ? old('company_id'): $expense->company_id }}">
                    @else
                        <input name="company_id" class="company-value" type="hidden" value="{{ old('_token') ? old('company_id'): '' }}">
                    @endif
                    <i class="dropdown icon"></i>
                    <div class="default text">choose a payer</div>
                    <div class="menu">
        ...
        <div class="col-md-3 branch-field" style="display: none;">
            <div class="form-group required">
                <div class="form-group required {{ $errors->has('branch_id') ? 'has-danger' : '' }}">
                    <label class="control-label">Pay to (Branch)</label>
                    <div class="ui fluid search normal selection dropdown company-drop-down {{ $errors->has('company_id') ? 'error' : '' }}">
                        @if(isset($expense))
                            <input name="branch_id" class="branch-value" type="hidden" value="{{ old('_token') ? old('branch_id'): $expense->branch_id }}">
                        @else
                            <input name="branch_id" class="branch-value" type="hidden" value="{{ old('_token') ? old('branch_id'): '' }}">
                        @endif
                        <i class="dropdown icon"></i>
                        <div class="default text">choose a branch</div>
                        <div class="menu">
                            @foreach(receiverCompanyDropDown() as $key => $branch)
                                <div class="item" data-value="{{ $key }}">{{ $branch }}</div>
                            @endforeach
                        </div>
                    </div>
                    <p class="form-control-feedback">{{ $errors->first('branch_id') }}</p>
                </div>
            </div>
        </div>
        <div class="col-md-3 shop-field" style="display: none;">
            <div class="form-group required">
                <div class="form-group required {{ $errors->has('shop_id') ? 'has-danger' : '' }}">
                    <label class="control-label">Pay to (Shop)</label>
                    <div class="ui fluid search normal selection dropdown company-drop-down {{ $errors->has('shop_id') ? 'error' : '' }}">
                        @if(isset($expense))
                            <input name="shop_id" type="hidden" value="{{ old('_token') ? old('shop_id'): $expense->shop_id }}">
                        @else
        ...
        <div class="col-md-3 approval-field" style="display: none;">
            <div class="form-group required">
                <label class="control-label">Approval required?</label>
                <div class="demo-radio-button">
                    <input name="approval_required" value="Yes" type="radio" class="with-gap approval-required"
                           id="Yes" {{ (old('approval_required') == 'Yes') || (isset($expense) && $expense->approval_required == 'Yes' ) ? 'checked' : ''}}>
                    <label for="Yes">Yes</label>
                    <input name="approval_required" value="No" type="radio" class="with-gap approval-required"
                           id="No" checked {{ (old('approval_required') == 'No') || (isset($expense) && $expense->approval_required == 'No' )  ? 'checked' : ''}}>
    ...
    <div class="row m-t-20">
        ...
        <div class="col-md-3">
            <div class="form-group required {{ $errors->has('expense_account') ? 'has-danger' : '' }}">
                ...
                <div class="ui fluid  search selection dropdown exp-acc-drop-down {{ $errors->has('expense_account') ? 'error' : '' }}">
                    @if(isset($expense))
                        <input name="expense_account" type="hidden" value="{{ old('_token') ? old('expense_account'): $expense->expense_account }}">
                    @else
                        <input name="expense_account" type="hidden" value="{{ old('_token') ? old('expense_account'): '' }}">
                    @endif
                    <i class="dropdown icon"></i>
                    <div class="default text">choose an expense account</div>
                    <div class="menu">
                        @foreach(accountsByCompanyDropDown() as $key => $expenseAcc)
                            <div class="item" data-value="{{ $key }}">{{ $expenseAcc }}</div>
                        @endforeach
                    </div>
                </div>
                <p class="form-control-feedback">{{ $errors->first('expense_account') }}</p>
            </div>
        </div>
        <div class="col-md-3">
