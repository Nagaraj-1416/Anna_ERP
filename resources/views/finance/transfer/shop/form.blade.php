    ...
    <div class="row m-t-10 cih-panel" style="display: none;">
        <div class="col-md-12">
            ...
            <table class="ui structured table collapse-table">
                ...
                <tbody>
                @if($cheques)
                    @foreach($cheques as $chequeNo => $cheque)
                        <tr>
                            <td style="width: 1%;">
                                <div class="demo-checkbox">
                                    <input type="checkbox" id="{{ 'md_checkbox_29_' . $chequeNo }}"
                                           name="cheques[]"
                                           class="chk-col-cyan" value="{{ $chequeNo }}" data-id="{{ $chequeNo }}" data-value="{{ getChequeDataByNo($chequeNo)['eachTotal'] }}">
                                    <label for="{{ 'md_checkbox_29_' . $chequeNo }}"></label>
                                </div>
                            </td>
                            <td style="width: 32%;">
                                <b>Cheque# </b><code style="font-size: 14px;">{{ $chequeNo }}</code><br />
                                <span class="text-warning">{{ getChequeDataByNo($chequeNo)['date'] }}</span>,
                                <span class="text-info">{{ getChequeDataByNo($chequeNo)['bank'] }}</span>
                            </td>
                            <td style="width: 47%;">
                                @if($cheque)
                                    @foreach($cheque as $chequeData)
                                        <div class="clearfix">
                                            <div class="pull-left">
                                                <a target="_blank" href="/sales/customer/{{ $chequeData->customer->id }}">
                                                    {{ $chequeData->customer->display_name }}
                                                </a><br />
                                                <a target="_blank" href="/sales/invoice/{{ $chequeData->chequeable->invoice_id }}">
                                                    {{ $chequeData->chequeable->invoice->ref }} ({{ $chequeData->chequeable->invoice->invoice_date }})
                                                </a>
                                            </div>
                                            <div class="pull-right">
                                                {{ number_format($chequeData->amount, 2) }}
                                            </div>
                                        </div>
                                        <br />
                                    @endforeach
                            ...
                            <td class="text-right" style="vertical-align: bottom; width: 20%;"><b>{{ number_format(getChequeDataByNo($chequeNo)['eachTotal'], 2) }}</b></td>
                        </tr>
                    @endforeach
                @else
                    <tr>
