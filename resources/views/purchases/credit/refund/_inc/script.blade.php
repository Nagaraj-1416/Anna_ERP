...
<script>
    app.controller('RefundController', function ($scope, $http) {
        ...
        // store refund
        $scope.refundStoreRoute = '{{ route('purchase.credit.refund.save', ['credit' => $credit]) }}';
        ...
        $scope.refundEditRoute = '{{ route('purchase.credit.refund.edit', ['credit' => $credit, 'refund' => 'ID']) }}';
        $scope.editRefund = function ($event) {
            ...
            $http.get($scope.refundEditRoute.replace('ID', refundId)).then(function (response) {
                ...
                if ($scope.refund.account) {
                    $scope.setDropDownValue($scope.el.refundedFromDD, $scope.refund.refunded_to, $scope.refund.account.name);
                }
        ...
        $scope.refundUpdateRoute = '{{ route('purchase.credit.refund.update', ['credit' => $credit, 'refund' => 'ID']) }}';
        ...
        $scope.refundDeleteRoute = '{{ route('purchase.credit.refund.delete', ['credit' => $credit, 'refund' => 'ID']) }}';
        ...
        $scope.getCreditRemain = function () {
            var limit = '{{ getSupplierCreditLimit($credit) }}';
            if ($scope.edit) {
