<script>
    app.controller('BillController', function ($scope, $http) {
        $scope.el = {
            sidebar: $('#bill-sidebar'),
            loader: $('.cus-create-preloader'),
            btn: $('.bill-sidebar-btn'),
        ...
        $scope.paymentEditRoute = '{{ route('purchase.payment.edit', ['payment' => 'ID']) }}';
        $scope.editPayment = function ($event) {
            ...
            $http.get($scope.paymentEditRoute.replace('ID', paymentId)).then(function (response) {
                ...
                $scope.payment = {
                    ...
                    paid_through: data.paid_through.id,
                    id: data.id,
                    total: data.payment
                };
                $scope.el.typeDD.dropdown('set text', $scope.payment.payment_type).dropdown('set value', $scope.payment.payment_type);
                $scope.el.accountDD.dropdown('set text', data.paid_through.name).dropdown('set value', data.paid_through.id);
            })
        };
        $scope.paymentUpdateRoute = '{{ route('purchase.payment.credit.update', ['payment' => 'ID']) }}';
        ...
        $scope.el.accountDD.dropdown('setting', {
            onChange: function (val) {
                $scope.payment.paid_through = val;
            }
        });
