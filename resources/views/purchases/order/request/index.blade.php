                ...
                <div class="contact-page-aside">
                    ...
                    <div class="left-aside">
                        <ul class="list-style-none">
                            <li class="text-muted m-t-20">FILTER REQUESTS BY</li>
                            <li class="divider"></li>
                            <li ng-class="{'active': !query.filter}" ng-click="filterUpdate()"><a
                                        href="">All Requests</a></li>
                        ...
                        <div class="right-page-header">
                            <div class="d-flex m-b-10">
                                <div class="align-self-center">
                                    <h2 class="card-title m-t-10">Requests
                                        @{{ pagination.total ? ("(" + pagination.total +")") :
                                        '' }}</h2>
                                </div>
                                <div class="ml-auto">
                                    <input type="text" id="demo-input-search2" ng-model="searchOrders"
                                           placeholder="search for requests here" class="form-control"
                            ...
                            <div class="col-md-12 m-b-20">
                                <table id="demo-foo-addrow" class="table m-t-10 table-hover no-wrap contact-list"
                                    ...
                                    <tr ng-repeat="request in requests">
                                        <td>
                                            @{{ request.request_no }}<br/>
                                            <small>
                                                <i class="mdi mdi-calendar"></i> @{{ request.request_date | date }}
                                            </small>
                                        </td>
                                        <td>
                                            @{{ request.store.name }}
                                        ...
                                        <td>
                                            <span ng-class="request.status == 'Closed' ? 'text-green' : '' ||
                                                request.status == 'Open' ? 'text-warning' : '' || request.status == 'Awaiting Approval' ? 'text-warning' : '' ||
                                                 request.status == 'Canceled' ? 'text-danger' : ''">
                                                <i ng-if="request.status == 'Closed'"
                                                   class="ti-check"></i>
                                                <i ng-if="request.status == 'Open'"
                                                   class="ti-truck"></i>
                                                @{{ request.status }}
                                            </span>
                                        </td>
                                        <td class="text-center">
                                            <a ng-if="request.status == 'Drafted'" title="View request details" class="btn btn-sm btn-info"
                                               href="/purchase/request/@{{ request.id }}/confirm">
                        ...
                        <div class="row" ng-hide="loading"
                            ...
                            <div class="col-md-12">
                                <div class="card text-center">
                                    <div class="card-body">
                                        <p class="card-text">There are <code>no</code> Purchase Requests found.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
...
@section('script')
    <script>
        app.controller('PurchaseOrderController', ['$scope', '$http', function ($scope, $http) {
            var moduleRoute = '{{ route('purchase.order.request') }}';
            $scope.orders = [];
            $scope.loading = true;
            $scope.filterd = false;
            $scope.pagination = {};
