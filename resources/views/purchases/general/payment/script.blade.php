<script>
    ...
    var route = '{{ route('purchase.payment.edit', ['payment' => 'PAYMENT']) }}';
    var update = '{{ route('purchase.payment.update', ['payment' => 'PAYMENT']) }}';
        ...
        if (editId) {
            $.ajax(route.replace('PAYMENT', editId), {
                ...
                $.each(response, function (key, val) {
                    ...
                    if (key === 'paid_through' && val) {
                        $(id).parent().dropdown('set text', val.name).dropdown('set value', val.id);
                        $(id).val(val.id)
                    } else {
                        $(id).val(val)
                    }
    ...
    function getVal() {
        return {
            ...
            cheque_bank_id: $('#cheque_bank_id').val(),
            account_no: $('#account_no').val(),
            deposited_date: $('#deposited_date').val(),
            dd_bank_id: $('#dd_bank_id').val(),
    ...
    deleteBtn.click(function () {
        var id = $(this).data('id');
        var deleteUrl = '{{ route('purchase.payment.delete', [ 'payment'=>'ID']) }}';
