            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body p-b-0">
                        <h3 class="card-title"><i class="ti-bar-chart"></i> Payments Made</h3>
                    </div>
                    <hr>
                    <div class="card-body">
                        <!-- from to filter -->
                        ...
                        <div class="heading-section">
                            <h2 class="text-center"><b>Payments Made</b></h2>
                                        ...
                                        <td colspan="8" class="no-padding-tbl-cel">
                                            ...
                                            <table class="table no-border">
                                                ...
                                                <tr ng-repeat="payment in datas" ng-show="length">
                                                    <td width="10%">
                                                        <a target="_blank"
                                                           href="{{ url('/') }}/purchase/bill/@{{ payment.bill.id }}">
                                                    ...
                                                    <td width="15%">@{{ payment.paid_through ? payment.paid_through.name
                                                        : 'None'}}
                                                    </td>
    ...
    <script>
        app.controller('purchaseProductController', ['$scope', '$http', function ($scope, $http) {
            ...
            $scope.generate = function () {
                ...
                $scope.loading = true;
                var orderRoute = '{{ route('report.payments.made') }}';
            ...
            $scope.getOrderBillTotal = function () {
                var bills = _.pluck($scope.orders, 'bills');
                var amounts = _.pluck(bills, 'amount');
                var amount = $scope.sum(amounts);
            };

            $scope.getSupplier = function (id) {
                var name = _.find($scope.suppliers, function (value, key) {
                    if (value.id === parseInt(id)) return value;
                });
                return name;
            };
            $scope.getExportRoute = function () {
                var route = '{{ route('report.payments.made.export') }}';
